...
class PageLayout(object):
    ...
    def render_to_image(self, image, thickness: int = 2, circles: bool = True,
                        render_order: bool = False, render_region_id: bool = False):
        ...
        if render_order or render_region_id:
            ...
            for idx, region in enumerate(self.regions):
                ...
                if render_order:
                    ...
                    cv2.putText(image, text, mid_coords, font, font_scale,
                                color=(0, 0, 0), thickness=font_thickness, lineType=cv2.LINE_AA)
                if render_region_id and region.id is not None:  # and region.category is not "text":
                    text = f"{idx},{region.id}"
                    text_w, text_h = cv2.getTextSize(text, font, font_scale, font_thickness)[0]
                    start_point = (int(min[0]), int(min[1]))
                    end_point = (int(min[0]) + text_w, int(min[1]) - text_h)
